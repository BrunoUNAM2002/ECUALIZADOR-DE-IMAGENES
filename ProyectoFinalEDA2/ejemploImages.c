...
void splitChannels()
{
  ...
  for (int i = 0; i < imaSize; i++)
  {
    ...
    imaBlue[i * channels + 2] = (r + g + b) / 3;

    imaRed[i * channels + 0] = 0;
    imaRed[i * channels + 1] = r;
    imaRed[i * channels + 2] = 0;

    imaGreen[i * channels + 0] = g;
    imaGreen[i * channels + 1] = 0;
    imaGreen[i * channels + 2] = 0;
  }

  // Saving image
  stbi_write_jpg("ProyectoFinalEDA2/output/imaBlue.jpg", width, height, 3, imaBlue, 100);
  stbi_write_jpg("ProyectoFinalEDA2/output/imaRed.jpg", width, height, 3, imaRed, 100);
  stbi_write_jpg("ProyectoFinalEDA2/output/imaGreen.jpg", width, height, 3, imaGreen, 100);
